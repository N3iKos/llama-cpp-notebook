...
def download_with_aria_live(url, out_dir, out_name=None, *, token="", connections=16, clear=True):
    ...
    before = local_size(out_path)
    if remote_size and before >= remote_size and before > 0:
        live_print(f"{out_name}\ncomplete: {before/1024**3:.2f} GiB", clear=clear)
        return str(out_path)

    headers = []
    if token:
        headers += ["--header", f"Authorization: Bearer {token}"]
    cmd = ["aria2c", "-c", "-x", str(connections), "-s", str(connections), "-k", "1M", "--file-allocation=none", "--allow-overwrite=true", "--auto-file-renaming=false", "--summary-interval=1", "--console-log-level=notice", "-d", str(out_dir), "-o", out_name, *headers, url]
    ...
    try:
        ...
        if remote_size and final_size < remote_size:
            raise RuntimeError(f"Incomplete download: {out_name}")
...
def default_config_path():
    if Path("/kaggle/working").exists():
        return Path("/kaggle/working/model_config.json")
    if Path("/content").exists():
        return Path("/content/model_config.json")
...
def download_model_pair(model_url, mmproj_url, model_dir, *, hf_token="", connections=16):
    ...
    mmproj_path = download_with_aria_live(mmproj_url, model_dir, token=hf_token, connections=connections) if mmproj_url else ""
    cfg = {"model_path": model_path, "mmproj_path": mmproj_path, "model_dir": str(model_dir), "model_url": model_url, "mmproj_url": mmproj_url}
