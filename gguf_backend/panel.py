...
class NotebookPanel:
    ...
    def _terminal_html(self, current=""):
        ...
        rows = "".join(f'<div style="min-height:1.35em;white-space:pre-wrap;word-break:break-word;overflow-wrap:anywhere;">{esc(line)}</div>' for line in reversed_lines)
  ...
  <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;padding:7px 10px;background:#101010;border-bottom:1px solid #333;color:#bbb;"><div>{esc(meta)}</div></div>
  <div id="{self.term_id}" style="height:{self.height}px;overflow-y:auto;padding:10px;white-space:pre-wrap;word-break:break-word;overflow-wrap:anywhere;background:#050505;color:#e8e8e8;display:flex;flex-direction:column-reverse;">{rows}</div>
</div>'''
    ...
    def set_links(self, title: str, links: dict[str, str], *, note: str = ""):
        ...
        for label, item in links.items():
            if isinstance(item, dict):
                url = str(item.get("url", "")); can_copy = bool(item.get("copy", True)); can_open = bool(item.get("open", False))
            else:
                url = str(item); can_copy = True; can_open = str(label).lower().find("public") >= 0
            input_id = f"inp_{uuid.uuid4().hex}"
            safe_url = html.escape(url, quote=True)
            actions = []
            if can_open:
                actions.append(f'<a href="{safe_url}" target="_blank" rel="noopener noreferrer" style="background:#1f2937;color:#f8fafc;border:1px solid #64748b;border-radius:16px;padding:6px 12px;text-decoration:none;cursor:pointer;">open</a>')
            if can_copy:
                actions.append(f'<button onclick="(async()=>{{const i=document.getElementById(\'{input_id}\'); if(i){{try{{await navigator.clipboard.writeText(i.value); this.textContent=\'copied\'; setTimeout(()=>this.textContent=\'copy\',1200);}}catch(e){{i.select();document.execCommand(\'copy\');}}}}}})()" style="background:#222;color:#eee;border:1px solid #555;border-radius:16px;padding:6px 12px;cursor:pointer;">copy</button>')
            cards.append(f'<div style="display:flex;gap:8px;align-items:center;margin:7px 0;"><div style="min-width:145px;color:#bbb;">{esc(label)}</div><input id="{input_id}" value="{safe_url}" readonly style="flex:1;background:#050505;color:#e8e8e8;border:1px solid #444;padding:7px;font-family:inherit;font-size:12px;min-width:120px;" /><div style="display:flex;gap:6px;align-items:center;">{"".join(actions)}</div></div>')
        self.set_dashboard("".join(cards))

    def section(self, text):
        self.append(""); self.append("═" * 80); self.append(text); self.append("═" * 80); self.render()
...
class SilentPanel:
    def __init__(self): self.lines = []; self.show_progress = False
    ...
    def append(self, line, *, max_lines=None):
        if line: self.lines.append(str(line)); self.lines = self.lines[-(max_lines or 500):]
...
def parse_aria2_progress(line: str):
    ...
    if m: data["size"] = f"{m.group(1)} / {m.group(2)}"
    return data


def update_download_progress(panel, line: str) -> bool:
    data = parse_aria2_progress(line)
    if data["pct"] is None: return False
    pct = data["pct"]
    parts = [f"{pct:.1f}%"]
    if data["size"]: parts.append(data["size"])
    if data["dl"]: parts.append(f"DL {data['dl']}")
    if data["eta"]: parts.append(f"ETA {data['eta']}")
    if data["conn"]: parts.append(f"CN {data['conn']}")
    if data["seed"]: parts.append(f"SEED {data['seed']}")
    info = " | ".join(parts)
...
def run_command(cmd, *, label="process", env=None, cwd=None, timeout=None, check=False, mode="terminal", log_name=None, log_dir=None, tail_lines=500, refresh_interval=0.25, hide_redirects=True, show=True, panel=None, finalize=True):
    log_dir = Path(log_dir) if log_dir else default_log_dir(); log_dir.mkdir(parents=True, exist_ok=True)
    safe_label = _safe_filename(label); log_path = Path(log_dir) / (log_name or f"{int(time.time())}_{safe_label}.log")
    if panel is None:
        panel = NotebookPanel(label, show_progress=(mode == "download")) if show else SilentPanel()
        if show: panel.display_panel()
    else: panel.display_panel()
    panel.set_progress_visible(mode == "download"); panel.set_footer(f"log: {log_path}"); panel.set_status("starting download..." if mode == "download" else f"running: {label}")
    if mode == "download": panel.set_progress(0, "0%", "")
    panel.append("$ " + command_text(cmd), max_lines=tail_lines); panel.render()
    shell = isinstance(cmd, str); env2 = os.environ.copy();
    if env: env2.update(env)
    start = time.time(); stdout_chunks=[]; current=""; last_refresh=0.0
    proc = subprocess.Popen(cmd, shell=shell, cwd=cwd, env=env2, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, stdin=subprocess.DEVNULL, bufsize=0)
    fd = proc.stdout.fileno() if proc.stdout else None
    if fd is not None: os.set_blocking(fd, False)
    try:
        with open(log_path, "w", encoding="utf-8", errors="replace") as log:
            while True:
                if timeout and time.time() - start > timeout:
                    proc.kill(); panel.finish(False, f"timeout after {timeout}s"); raise TimeoutError(f"{label} timeout after {timeout}s")
                ready=[]
                if fd is not None: ready,_,_ = select.select([fd], [], [], 0.1)
                if ready:
                    try: raw=os.read(fd,8192)
                    except BlockingIOError: raw=b""
                    if raw:
                        chunk=raw.decode("utf-8", errors="replace"); stdout_chunks.append(chunk); log.write(chunk); log.flush()
                        chunk=strip_ansi(chunk).replace("\r","\n"); pieces=chunk.split("\n"); pieces[0]=current+pieces[0]; current=pieces[-1]
                        for raw_line in pieces[:-1]:
                            line=raw_line.strip()
                            if not line: continue
                            if mode=="download": update_download_progress(panel,line)
                            clean=sanitize_log_line(line, hide_redirects=hide_redirects)
                            if clean: panel.append(clean, max_lines=tail_lines)
                now=time.time()
                if now-last_refresh>=refresh_interval:
                    if mode != "download": panel.set_status(f"running: {label} | elapsed {int(now-start)}s")
                    panel.render(sanitize_log_line(current, hide_redirects=hide_redirects) or ""); last_refresh=now
                if proc.poll() is not None: break
    finally:
        try:
            if proc.poll() is None: proc.kill()
        except Exception: pass
    rc=proc.wait(); stdout="".join(stdout_chunks); panel.append(f"$ exit {rc}", max_lines=tail_lines)
    if rc==0:
        panel.set_status("download completed" if mode=="download" else f"completed: {label}")
        if mode=="download": panel.set_progress(100,"100% | completed","success")
        panel.render();
        if finalize: panel.finish(True, "download completed" if mode=="download" else f"completed: {label}")
    else:
        panel.append("", max_lines=tail_lines); panel.append("--- tail log ---", max_lines=tail_lines)
        for line in tail_text(log_path, 80).splitlines():
            clean=sanitize_log_line(line, hide_redirects=hide_redirects)
            if clean: panel.append(clean, max_lines=tail_lines)
        panel.render();
        if finalize: panel.finish(False, f"failed: {label} | exit {rc}")
    result=subprocess.CompletedProcess(cmd, rc, stdout=stdout, stderr=None); result.log_path=str(log_path)
    if check and rc != 0: raise subprocess.CalledProcessError(rc, cmd, output=stdout)
    return result
...
def show_summary(title: str, data=None, *, lines: Optional[Iterable[str]]=None, log_path=None, panel=None, finalize=True):
    ...
    if panel is None:
        panel = NotebookPanel(title, show_progress=False, height=220); panel.display_panel()
    if lines is None:
        if isinstance(data, dict): lines=[f"{k}: {v}" for k,v in data.items()]
        elif data is None: lines=[]
        else: lines=[str(data)]
    if log_path: panel.set_footer(f"log: {log_path}")
    panel.set_summary(title, lines=lines)
    if own_panel:
        panel.set_lines([str(line) for line in lines]); panel.render()
        if finalize: panel.finish(True, "summary")
    return panel
