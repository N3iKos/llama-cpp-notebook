...
def build_cmd(cfg: ServerConfig, help_text, server):
    ...
    if cfg.flash_attn not in (None, "", False) and has_flag(help_text, "--flash-attn"):
        fa = str(cfg.flash_attn).strip().lower()
        if fa in ("on", "off", "auto"):
            cmd += ["--flash-attn", fa]
        elif cfg.flash_attn is True:
            cmd += ["--flash-attn", "on"]
    ...
    if cfg.thinking_config:
        ...
        if cfg.jinja and has_flag(help_text, "--jinja"):
            cmd += ["--jinja"]
        if cfg.reasoning is not None and has_flag(help_text, "--reasoning"):
            cmd += ["--reasoning", str(cfg.reasoning)]
        if cfg.reasoning_budget is not None and has_flag(help_text, "--reasoning-budget"):
            cmd += ["--reasoning-budget", str(cfg.reasoning_budget)]
        if cfg.reasoning_format is not None and has_flag(help_text, "--reasoning-format"):
            cmd += ["--reasoning-format", str(cfg.reasoning_format)]
    if has_flag(help_text, "--log-colors"):
        cmd += ["--log-colors", "off"]
    if cfg.extra_server_args:
        cmd += [str(x) for x in cfg.extra_server_args]
