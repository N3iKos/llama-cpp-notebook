...
def build_cmd(cfg: ServerConfig, help_text, server):
    ...
    if cfg.mmproj_path and Path(cfg.mmproj_path).exists():
        ...
        if cfg.mmproj_offload and has_flag(help_text, "--mmproj-offload"):
            cmd += ["--mmproj-offload"]
    cmd += ["--host", cfg.host, "--port", str(cfg.port), "--alias", cfg.alias, "-c", str(cfg.ctx_size), "-t", str(cfg.threads), "-ngl", str(cfg.gpu_layers)]
    ...
    while time.time() - start < timeout:
        ...
        if port_is_open("127.0.0.1", cfg.port):
            try:
                ...
                pass
        if time.time() - last >= 1:
            live_print("\n".join(["llama-server starting", f"elapsed: {int(time.time()-start)}s", f"port: {cfg.port}", "", tail_text(log_path, 20)]), clear=True)
            last = time.time()
        time.sleep(0.5)
...
def start_server(cfg: ServerConfig, *, warmup=True):
    ...
    live_print("\n".join(["llama-server ready", f"local: {base}/v1/chat/completions", f"model: {cfg.alias}", f"pid: {proc.pid}"]), clear=True)
    return {"base_url": base, "chat_endpoint": base + "/v1/chat/completions", "models_endpoint": base + "/v1/models", "pid": proc.pid, "cmd": cmd, "log_path": str(log_path)}
